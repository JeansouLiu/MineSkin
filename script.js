...
mineskinApp.controller("indexController", ["$scope", "Upload", "$state", "$http", "$timeout", "$interval", "$stateParams", "$localStorage", function ($scope, Upload, $state, $http, $timeout, $interval, $stateParams, $localStorage) {
    ...
    $scope.generate = function () {
        ...
        if ($scope.skinUrl) {
            ...
            $scope.generateProgress = 0;
            var genAlert = $scope.addAlert("从 URL 创建皮肤中...", "info", 15000);
            setTimeout(function () {
                $http({
            ...
            if ($scope.skinUser.length > 16) {// Possibly a UUID
                ...
                var validateAlert = $scope.addAlert("验证用户名中...", "info", 10000);
                $.ajax({
    ...
    $scope.generateSuccess = function (data, genAlert) {
        ...
        $scope.generateAttempt = 0;
        var successAlert = $scope.addAlert("皮肤创建成功!", "success", 10000);
        if (genAlert) {
            genAlert.close();
        }
    ...
    $scope.generateError = function (message, genAlert) {
        ...
        $scope.addAlert("创建皮肤失败: " + message, "danger", 2000);
        if (genAlert) {
            genAlert.close();
        }
        ...
        $timeout(function () {
            Materialize.toast("等待 5 秒后重试...", 5000);
            $timeout(function () {
                $scope.generate();
    ...
    $scope.accountStatus = function () {
        $http({
            ...
            if (response.data.passwordUpdated) {
                Materialize.toast("账户密码已更新");
            }
            if (response.data.securityUpdated) {
                Materialize.toast("安全提问已更新");
            }
        }, function (response) {
            console.log(response);
            if (response.data.error) {
